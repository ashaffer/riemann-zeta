...
| `FloorWitness.*` (4 thms) | **first kernel-checked IMPOSSIBILITY artifact** (2026-07-26 night, IAS proof-theory seat's spec): at (L, m) = (711/200, 40) NO entrywise 1e−20-ball certificate of the house window form exists — rank-one adversary M = A − δssᵀ, one exact-ℤ kernel inequality; `no_ball_certificate_ge` (∀ δ' ≥ 1e−20), `depth_law_bracket` (formal regression of the δ* = λ/‖v‖₁² depth law into (1.5, 1.6)e−21), 6.5× headroom; coordinator-countersigned incl. the frame-bookkeeping deviation ruling | `FloorWitness.lean` |
...
| `HardHorizon.hard_horizon` + `zero_desert` + `anchor_collapse`(`_of_deep`) | **first ANALYTIC kernel-checked theorems** (2026-07-26 night): the Hard Horizon Theorem T1′ (staircase form, full multiplicity) — anchored vanishing on the rigid multiset cannot extend past ln(T̃/2π) ≤ 2a+2+ε*, ε* explicit — the zero-desert Corollary 2 (stronger than the paper form), and the annihilating-pair form `anchor_collapse` (IAS quasicrystal seat's spec, formalized same session, stronger than spec'd: anchor hypothesis dropped via case split, explicit collapse bound past the horizon); 40 declarations; Corollary 1 (ζ unconditional) blocked on formalized RvM/S(T) only | `lean/glide/Glide/HardHorizon.lean` |
